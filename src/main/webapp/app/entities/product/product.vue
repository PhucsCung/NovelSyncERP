<template>
  <div>
    <h2 id="page-heading" data-cy="ProductHeading">
      <span v-text="$t('novelSyncErpApp.product.home.title')" id="product-heading">Products</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('novelSyncErpApp.product.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ProductCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('novelSyncErpApp.product.home.createLabel')"> Create a new Product </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && products && products.length === 0">
      <span v-text="$t('novelSyncErpApp.product.home.notFound')">No products found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('sku')">
              <span v-text="$t('novelSyncErpApp.product.sku')">Sku</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'sku'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('novelSyncErpApp.product.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('basePrice')">
              <span v-text="$t('novelSyncErpApp.product.basePrice')">Base Price</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'basePrice'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('attributes')">
              <span v-text="$t('novelSyncErpApp.product.attributes')">Attributes</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'attributes'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('category.name')">
              <span v-text="$t('novelSyncErpApp.product.category')">Category</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="novelSyncErpApp.product.delete.question" data-cy="productDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-product-heading" v-text="$t('novelSyncErpApp.product.delete.question', { id: removeId })">
