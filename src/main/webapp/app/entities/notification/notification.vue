<template>
  <div>
    <h2 id="page-heading" data-cy="NotificationHeading">
      <span v-text="$t('novelSyncErpApp.notification.home.title')" id="notification-heading">Notifications</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('novelSyncErpApp.notification.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'NotificationCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('novelSyncErpApp.notification.home.createLabel')"> Create a new Notification </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && notifications && notifications.length === 0">
      <span v-text="$t('novelSyncErpApp.notification.home.notFound')">No notifications found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="notifications">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="$t('novelSyncErpApp.notification.title')">Title</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('message')">
              <span v-text="$t('novelSyncErpApp.notification.message')">Message</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'message'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('isRead')">
              <span v-text="$t('novelSyncErpApp.notification.isRead')">Is Read</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'isRead'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('type')">
              <span v-text="$t('novelSyncErpApp.notification.type')">Type</span>
            ...
            <th scope="row" v-on:click="changeOrder('referenceId')">
              <span v-text="$t('novelSyncErpApp.notification.referenceId')">Reference Id</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'referenceId'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('createdAt')">
              <span v-text="$t('novelSyncErpApp.notification.createdAt')">Created At</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'createdAt'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('recipient.fullName')">
              <span v-text="$t('novelSyncErpApp.notification.recipient')">Recipient</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'recipient.fullName'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="notification in notifications" :key="notification.id" data-cy="entityTable">
            ...
            <td v-text="$t('novelSyncErpApp.NotificationType.' + notification.type)">{{ notification.type }}</td>
            <td>{{ notification.referenceId }}</td>
            <td>{{ notification.createdAt ? $d(Date.parse(notification.createdAt), 'short') : '' }}</td>
            <td>
              <div v-if="notification.recipient">
                <router-link :to="{ name: 'EmployeeView', params: { employeeId: notification.recipient.id } }">{{
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="novelSyncErpApp.notification.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-notification-heading" v-text="$t('novelSyncErpApp.notification.delete.question', { id: removeId })">
