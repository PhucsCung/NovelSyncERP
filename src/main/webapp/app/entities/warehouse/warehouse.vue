<template>
  <div>
    <h2 id="page-heading" data-cy="WarehouseHeading">
      <span v-text="$t('novelSyncErpApp.warehouse.home.title')" id="warehouse-heading">Warehouses</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('novelSyncErpApp.warehouse.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'WarehouseCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('novelSyncErpApp.warehouse.home.createLabel')"> Create a new Warehouse </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && warehouses && warehouses.length === 0">
      <span v-text="$t('novelSyncErpApp.warehouse.home.notFound')">No warehouses found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('code')">
              <span v-text="$t('novelSyncErpApp.warehouse.code')">Code</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'code'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('novelSyncErpApp.warehouse.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('address')">
              <span v-text="$t('novelSyncErpApp.warehouse.address')">Address</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="novelSyncErpApp.warehouse.delete.question" data-cy="warehouseDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-warehouse-heading" v-text="$t('novelSyncErpApp.warehouse.delete.question', { id: removeId })">
